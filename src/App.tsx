...
function TabLink({ to, label, icon: Icon }: { to: string; label: string; icon: any }) {
  return (
    <NavLink to={to} className={({ isActive }: { isActive: boolean }) => `flex flex-col items-center gap-1 px-3 py-2 rounded-lg ${isActive ? 'text-brand-600 bg-brand-600/10' : 'text-gray-600 dark:text-gray-300'}`}>
